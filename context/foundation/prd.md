...
For MVP, the score should reflect a rolling two-week window rather than all-time history, so the result stays current enough to feel fair while avoiding extreme day-to-day swings. Fairness Score is defined as each member's percentage share of all chore points completed by the household during that rolling two-week window.
